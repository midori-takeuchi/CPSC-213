...
struct Washroom* createWashroom() {
  ...
  washroom->female_count = 0;
  washroom->male_count = 0;
  washroom->isFull = uthread_cond_create(washroom->mutex);
  washroom->isEmpty = uthread_cond_create(washrom->mutex);
  return washroom;
}
